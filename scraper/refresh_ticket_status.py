...
calls. Runs once a day during August as the `refresh-tickets` scheduled task; a
no-op on any date outside the festival. Run it by hand (see Usage below) when a
date needs fresher status than the daily pass gives it.
...
catalogue for days while still picking up each day's refreshed availability.
