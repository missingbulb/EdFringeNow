"use strict";
const { jerusalemReady, calendarDays, calendarSpans, moveTripEnd } = require("../../shared/case-helpers");

/* The reader sets the trip: a handle dragged along the year, a handle stepped
 * with the keys, and the calendar, the address and the stored trip follow each
 * time. A trip past the cap pulls its other end along. */
module.exports = {
  description: "the trip's first and last day are dragged on the timeline or stepped with the keys, and the calendar follows",
  ...
  async verify(page, { origin, assert }) {
    ...
    assert.equal(await page.textContent(".tl-day--to"), "24", "the day written by the handle follows it");
    ...
    assert.equal(url.searchParams.get("from"), "2026-10-14", "the address follows the trip");
    assert.equal(url.searchParams.get("to"), "2026-10-24");
    assert.deepEqual(
      await page.evaluate(() => JSON.parse(localStorage.getItem("planNG.trip"))),
      { from: "2026-10-14", to: "2026-10-24", pick: "jerusalem-comedy@2026" },
      "and so does the stored trip, Jerusalem still chosen"
    );

    // A last day past the cap pulls the first day after it.
    await moveTripEnd(page, "to", "2026-11-20");
    const capped = new Date(Date.UTC(2026, 10, 20 - (MAX_PERIOD_DAYS - 1))).toISOString().slice(0, 10);
    await calendarSpans(page, capped, "2026-11-20");
