"use strict";
const { openOthers, jerusalemAllDays, jerusalemReady, settle } = require("../../shared/case-helpers");

// Monday at 22:00: a one-night show took the hour from a free late-night that
// plays four evenings. Taking that one from the hour's list is a lock, and a lock is
...
module.exports = {
  ...
  async capture(page, t) {
    const slot = page.locator(`${NIGHT} .sch-slot--contested`).first();
    const frame = async () => t.clip(t.pad(await slot.boundingBox(), 8));
    const before = await frame();

    await openOthers(page, slot);
