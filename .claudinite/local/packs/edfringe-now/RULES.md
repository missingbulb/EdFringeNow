...
task: `full-scrape`, `fetch-prices`, `price-probe`, `refresh-shows` or
`refresh-tickets`.
Never "verify" a scraper change by reasoning about what the API probably
returns — either have a sanctioned task run it, or say plainly that it is
unverified.
...
### A long-running job that commits generated data will race the hourly refresh

Any job that writes to `data/normalized/` or anything under `site/data/`
...
version. `commit_regenerated` in the task workers' shared `worker-lib.sh` is the
worked fix: on a rejected push, fetch `origin/main`, take its generated files,
regenerate from that master plus the run's own non-generated input, retry. A new
long-running writer of these paths calls it rather than growing a bespoke
recovery discovered the same way after the fact.
