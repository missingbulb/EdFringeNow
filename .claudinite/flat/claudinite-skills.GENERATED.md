...
| `scraper/**`, `data/**`, `site/data/**`, `.claudinite/local/packs/edfringe-now/tasks/full-scrape/**`, `.claudinite/local/packs/edfringe-now/tasks/fetch-prices/**`, `.claudinite/local/packs/edfringe-now/tasks/worker-lib.sh`, `site/js/app.js`, `site/js/clock.js`, `site/plan/plan.js`, `site/plan/lib/hydrate.js`, `site/plan/lib/availability.js`, `site/shared/price.js`, `site/shared/data-cache.js` | `data-pipeline` | edfringe-now | EdFringeNow's data pipeline - getting show data out of edfringe.com into the committed files the site serves, and the wire format the pages decode. Use before touching scraper/, data/, site/data/, the scrape and price tasks, or the site code that decodes, prices or times performances. |
...
| `**/wrangler.json`, `**/wrangler.jsonc`, `**/tasks/site-release/**` | `releasing-a-cloudflare-site` | cloudflare-site | How a Cloudflare-served site is released, forced and rolled back. Use when changing the release task, the wrangler config or the analytics loader, and when a release parks. |
| `product/requirements.md`, `product/requirements/**` | `requirements-harness` | edfringe-now | How EdFringeNow runs product/requirements.md as tests under a real headless Chromium - writing leaves and goldens, the harness's determinism traps, the frozen fixtures and golden approval. Use before editing product/requirements.md or anything under product/requirements/. |
...
| `product-wiki/**` | `writing-wiki-pages` | product-wiki | How a product-wiki page is written and grown: key insights, citation, what a source supports, correction without deletion. Loaded for any edit under product-wiki/. |
...
| `explore-link` | product-wiki | Mine one URL for product, market, usage and pricing insights and fold them into a product-wiki page. Use on "/explore-link <url>", or when handed a link for the wiki. |
